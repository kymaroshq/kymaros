...
func main() {
	...
	// JSON structured logging for slog (used by our controllers)
	...
	// License manager — watches the kymaros-license Secret for live updates.
	licMgr := license.NewManager(mgr.GetClient())
	licMgr.Refresh(context.Background())

	if err := (&license.LicenseReconciler{
		Client:  mgr.GetClient(),
		Manager: licMgr,
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to create controller", "controller", "License")
		os.Exit(1)
	}
