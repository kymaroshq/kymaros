...
func TestHandleHealth(t *testing.T) {
	q := newTestQueries()
	w := doRequest(HandleHealth(q), http.MethodGet, "/api/v1/health", nil)
...
func TestHandleCreateTestInvalidProvider(t *testing.T) {
	...
	input := CreateTestInput{Name: "test", Provider: "unsupported", Namespaces: []string{"staging"}}
	body, err := json.Marshal(input)
	require.NoError(t, err)
