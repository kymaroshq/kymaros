...
func NewQueries(c client.Client, lm *license.LicenseManager) *Queries {
	return &Queries{client: c, License: lm}
}

// GetLicenseResponse builds a LicenseResponse from the active license.
func (q *Queries) GetLicenseResponse() LicenseResponse {
	lic := q.License.Current()
	...
	if lic.ExpiresAt != nil {
		resp.ExpiresAt = lic.ExpiresAt.Format("2006-01-02")
	}
	if lic.TrialEndsAt != nil {
		resp.IsTrialing = true
		resp.TrialEndsAt = lic.TrialEndsAt.Format("2006-01-02")
		daysLeft := max(int(time.Until(*lic.TrialEndsAt).Hours()/24), 0)
...
// GetCompliance aggregates compliance data over the given period.
func (q *Queries) GetCompliance(ctx context.Context, framework string, days int) (ComplianceResponse, error) {
	tests, err := q.ListTests(ctx)
	if err != nil {
		return ComplianceResponse{}, err
	}
	...
	dailyScores, err := q.GetDailyScores(ctx, days)
	if err != nil {
		return ComplianceResponse{}, err
	}

	// Average score across all reports in the period.
	var totalScore float64
	var failures int
	for i := range reports {
		totalScore += float64(reports[i].Status.Score)
		if reports[i].Status.Result == restorev1alpha1.ResultFail {
			failures++
		}
	}
	avgScore := 0.0
	...
	// Days that had at least one test.
	daysWithTests := len(dailyScores)
	...
	nsCoverage := fmt.Sprintf("%d namespaces", len(nsSet))

	// Average RTO and target across reports.
	var totalRTO time.Duration
	var totalTarget time.Duration
	var rtoCount, targetCount int
	for i := range reports {
		measured := reports[i].Status.RTO.Measured.Duration
		if measured > 0 {
			totalRTO += measured
			rtoCount++
		}
		target := reports[i].Status.RTO.Target.Duration
		if target > 0 {
			totalTarget += target
			targetCount++
		}
	...
	rtoTarget := ""
	if targetCount > 0 {
		rtoTarget = formatDuration(totalTarget / time.Duration(targetCount))
	}
	rtoCompliant := rtoCount > 0 && targetCount > 0 &&
		(totalRTO/time.Duration(rtoCount)) <= (totalTarget/time.Duration(targetCount))

	// Determine overall status.
	status := "compliant"
	if failures > 0 || !rtoCompliant {
	...
	return ComplianceResponse{
		...
		Period:            period,
		Status:            status,
		TestsExecuted:     len(reports),
		AverageScore:      avgScore,
		NamespacesCovered: nsCoverage,
		DaysWithTests:     daysWithTests,
		DaysInPeriod:      days,
		IssuesDetected:    failures,
		AverageRTO:        avgRTO,
		RTOTarget:         rtoTarget,
		RTOCompliant:      rtoCompliant,
		DailyData:         dailyScores,
	}, nil
}
